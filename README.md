...
\- `time` — Millisecond timing via `ticks\_ms` and `ticks\_diff`
...
| `draw\_centered(text, center\_x, y)` | Renders text horizontally centered on a given x-axis |

| `draw\_players(score1, score2, countdown)` | Draws the main game screen with scores or countdown overlay |

| `draw\_timer(seconds\_left)` | Draws the countdown timer at the top-center of the screen |

| `start\_countdown()` | Runs the 3-2-1-Start pre-game sequence |

| `game\_loop()` | Main 10-second game: handles scoring, timer, and reset detection |

| `winner\_animation(score1, score2)` | Displays winner text and sparkle particle effect |
